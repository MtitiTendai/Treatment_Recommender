...
def login():
    if request.method == 'POST':
        ...
        cursor.execute("SELECT fullname, email, password FROM users WHERE email = %s", (email,))
        user = cursor.fetchone()
        cursor.close()

        if user:
            db_fullname, db_email, db_password = user
            
            if fullname != db_fullname:
                flash('Enter the correct fullname')
                return redirect('/login')
            
            if email != db_email:
                flash('Invalid email, pleas enter the correct username')
                return redirect('/login')
            ...
            if password == db_password:
                ...
                flash('Login failed, incorrect user email or password')
                return redirect('/login')
        else:
            flash('User not found, please register to login')
